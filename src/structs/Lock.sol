...
library LockLibrary {
    /// @notice Thrown when trying to check out an item while the lock it not unlocked
    ...
    function checkOut(Lock storage s_self, uint256 item) internal {
        ...
        // opt to loop a short array instead if supporting random access
        uint256[] memory checkedOutItems = s_self.checkedOutItems;
        uint256 len = checkedOutItems.length;
